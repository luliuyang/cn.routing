...
import cn.routing.runUtil.info.CrossInfo;
import cn.routing.runUtil.info.RoadInfo;
import com.sun.org.apache.xpath.internal.functions.FuncFalse;

import javax.xml.stream.FactoryConfigurationError;
import java.util.*;

import static cn.routing.runUtil.info.CrossInfo.getNormalizedRoadLeftLength;
...
//        }
    ...
    public static void UpdateCarsAtState1(String firstCarID, int t) {
        ...
        // firstCar所在的Road
        Road carInRoad = Main.MapRoad.get(firstCar.getRoadID());
        // firstCar行驶的路上有哪些lane
        LinkedList<Lane> carInLanes = new LinkedList<Lane>();
        ...
        if (firstCar.getCurFromCrossID().equals(carInRoad.getFromCrossID())) {
            ...
            // car行驶的路上有哪些lane
            carInLanes = carInRoad.getBackwardLane();
        }

        int carInLaneID = firstCar.getLaneID();
        // fisrtCar所在的lane
        Lane laneInvlovesCar = carInLanes.get(carInLaneID);
        ...
        if (firstCar.getCurToCrossID().equals(firstCar.getToCrossID())) {
            // 设置到家时间
            firstCar.setRealEndTime(t + 1);
            //显示一下到家的车
            System.out.println("The car " + firstCarID + " has arrived at its destination at time slot " + t + "!");
            // 放进ArrivalCars集合
            Main.ArrivedCar.add(firstCarID);
            // 把它从路上的车集合里面删掉
            Main.NowInRoadCar.remove(Main.MapCar.get(firstCarID));
            // $$$$$$将到家的Car从lane上去掉$$$$$$
            laneInvlovesCar.carsInLane.removeFirst();
        ...
        // **********情况2：能过路口，到别的路上
            ...
            // firstCar的行驶速度
            int firstCarSpeed = Math.min(firstCar.getMaxVelocity(), nextRoad.getMaxRoadVelocity());
            // firstCar去往的路上有哪些lane
            LinkedList<Lane> carInNextLanes = new LinkedList<Lane>();
            // 判断方向
            if (firstCar.getCurToCrossID().equals(nextRoad.getFromCrossID())) {
                carInNextLanes = nextRoad.getForwardLane();
            } else {
                // firstCar将要前往的路上有哪些lane
                carInNextLanes = nextRoad.getBackwardLane();
            }
            ...
            if (templaneID == -1) {
                ...
                // ------统一更新信息
                // $$$$$把car加到这个lane上$$$$$
                carInNextLanes.get(firstCar.getLaneID()).carsInLane.add(firstCar);
                // 更新标志位
    ...
    protected static void UpdateRoadForCarsAtState2(String firstCarID) {
        ...
        if (!firstCar.getNextRoadID().equals("-1")) {
            // 其实找到路了，只是道路限速过不去
            UpdateRoadForCarsAtState2Super(firstCarID);
        } else {
            // 本来就没能变成等待车
            UpdateRoadForCarsAtState2Nomal(firstCarID);
        }
    }
    ...
    protected static void UpdateRoadForCarsAtState2Super(String firstCarID) {
        ...
        // firstCar是由1车变过来的，curpos都变成0了，状态一定是1
        newFirstCar.setState(1);

        // 更新位置之后再设置hasArrangedOrNot
    ...
    protected static void UpdateRoadForCarsAtState2Nomal(String firstCarID) {
        ...
        Car newFirstCar = Main.MapCar.get(newFirstCarID);
        // firstCar此时的行驶速度
        int newmaxSpeed = Math.min(newFirstCar.getMaxVelocity(), road.getMaxRoadVelocity());
    ...
    protected static void UpdateLanePosForCarsAtState2(String firstCarID) {
        ...
        if (firstCar.getNextRoadID().equals("-1")) {
            ...
            // 由于限速过不去nextRoad的车，curPos设置为0
            firstCar.setCurPos(0);
    ...
    public static void carsFromGarageInsertToRoad(int t) {
        ...
        while (garageWaitSize > 0) {
            ...
            if(road.getRoadID().equals("5007"))
                System.out.println();
            /**用于拥塞控制*/
            float normalizedRoadLeftLength = getNormalizedRoadLeftLength(road, car.getCurFromCrossID());

            if (normalizedRoadLeftLength <= 1) {
                //每次都要清除的
                reArrangeCars.clear();
                if (road != null) {
